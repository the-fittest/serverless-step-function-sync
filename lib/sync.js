      ...
      if (key === 'States') {
        for (const [stateKey, state] of Object.entries(val)) {
          ...
          if (type === 'Task') {
            ...
            if (_.isString(_.first(_.get(resource, 'Fn::GetAtt')))) {
            ...
            } else if (_.isString(_.get(state, 'Parameters.StateMachineArn.Ref'))) {
            ...
            } else if (_.isString(_.get(state, 'Parameters.TableName.Ref'))) {
              const ref = _.get(state, 'Parameters.TableName.Ref');
              const tableName = _.find(resources, ['OutputKey', `${ref}Name`]);
              if (!tableName) {
                throw new Error(`Table ${ref} does not exist in stack resources`);
              }
              _.set(state, 'Parameters.TableName', tableName.OutputValue);
            }
